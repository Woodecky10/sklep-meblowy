...
import type { InquiryStatus } from "@/app/_lib/inquiries";

export type ActionResult =
  | { ok: true; message?: string }
  | { ok: false; error: string };

const ALLOWED_STATUSES: InquiryStatus[] = ["new", "read", "replied", "closed"];
...
export async function deleteInquiry(formData: FormData): Promise<ActionResult> {
  ...
  revalidatePath("/admin/zapytania");
  return { ok: true, message: "Zapytanie usunięte" };
}
