...
export default function GalleryBlock({ content }: { content: LocalizedGalleryContent }) {
  ...
  // kolumny, trzy i więcej pełną siatkę. Zwykły <img> zamiast next/image, bo
  // przy images.unoptimized=true (next.config) next/image nic nie optymalizuje,
  // a wymagałby width/height/fill — co blokuje naturalne proporcje.
      ...
      <div className={containerCls}>
        {images.map((img, i) => (
          <figure key={`${img.url}-${i}`} className={single ? "" : "mb-4 break-inside-avoid"}>
            ...
            />
            {img.alt && (
              <figcaption className="mt-2 text-sm text-[var(--muted)] font-sans">
                {img.alt}
              </figcaption>
            )}
          </figure>
