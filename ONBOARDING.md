...
- **Nazwa na ekranie zgody = `Mollien.pl`. Zgłoszona do weryfikacji 2026-07-30, JESZCZE NIE WIDOCZNA dla klientów.** Po publikacji do produkcji Google wymaga **weryfikacji marki**, żeby wyświetlać nazwę/logo/linki („Elementy marki → Stan weryfikacji"). Do tego czasu klient widzi na ekranie zgody surowy host `tlvgsddpiikolgdwuwmc.supabase.co`. **To nie blokuje logowania** — zakresy `email`/`profile` są niewrażliwe, więc nikt nie dostaje ostrzeżenia o niezweryfikowanej aplikacji; wpływa wyłącznie na wygląd. Decyzja przychodzi mailem na `miki19991@gmail.com`, po kilku dniach. **W trakcie przeglądu nie edytować pól na tym ekranie** — edycja wywala zgłoszenie na koniec kolejki.
  ...
  2. *„strona główna nie wyjaśnia celu aplikacji"* — ⚠️ **wcześniej było tu napisane, że nie mamy na to wpływu; to była nieprawda.** 2026-08-10 okazało się, że to JEDYNY powód, który po poprawkach został (zrzut z konsoli), i leżał w kodzie: home mówiła wyłącznie hasłami („Meble, które opowiadają historię"), więc recenzent nie miał z czego wyczytać, czym aplikacja jest ani po co jest w niej konto. Naprawione sekcją „O sklepie Mollien" na dole home (`app/page.tsx`, treść w słowniku `home.about*`) — zwykłym językiem: co sklep sprzedaje, jak działa zamawianie i **do czego służy konto zakładane przez Google**. Sekcja jest celowo poza systemem bloków z panelu, żeby nie dało się jej wyłączyć,
