...
- **Nazwa na ekranie zgody — ustawiona (`Mollien`), ale JESZCZE NIE WIDOCZNA dla klientów.** Po publikacji do produkcji Google wymaga **weryfikacji marki**, żeby wyświetlać nazwę/logo/linki („Elementy marki → Stan weryfikacji: aby elementy Twojej marki mogły być wyświetlane użytkownikom, musisz je zweryfikować"). Do tego czasu klient widzi na ekranie zgody surowy host `tlvgsddpiikolgdwuwmc.supabase.co`. **To nie blokuje logowania** — zakresy `email`/`profile` są niewrażliwe, więc nikt nie dostaje ostrzeżenia o niezweryfikowanej aplikacji; wpływa wyłącznie na wygląd.
  Łańcuch do odblokowania nazwy (kolejność wymuszona przez Google):
  1. potwierdzić własność `mollien.pl` w **Google Search Console** (rekord DNS w home.pl — patrz decyzja `www` vs apex niżej),
  2. dodać `mollien.pl` do *Autoryzowanych domen* (bez kroku 1 Google odrzuci) i wypełnić *Stronę główną* + *Link do polityki prywatności* (`/prywatnosc`) + *Link do warunków* (`/regulamin`),
  3. kliknąć **Zweryfikuj markę** (do tego czasu przycisk jest wygaszony) → przegląd po stronie Google, liczony w dniach.
